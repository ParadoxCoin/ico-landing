...
const MediaLibraryPage = () => {
  ...
  const { 
    ...
    ['mediaLibrary', page],
    () => mediaAPI.getAll(page, ITEMS_PER_PAGE),
    { keepPreviousData: true }
  );

  const { mutate: deleteMedia, isLoading: isDeleting } = useMutation(
    mediaAPI.delete,
    {
      onSuccess: () => {
  ...
  const { mutate: toggleShowcase, isLoading: isToggling } = useMutation(
    (params: { mediaId: string; isShowcase: boolean }) => 
      mediaAPI.toggleShowcase(params.mediaId, !params.isShowcase),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('mediaLibrary');
      },
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
              ...
              <Card key={item.id} className="overflow-hidden group">
                ...
                <CardFooter className="p-2 flex justify-between">
                  <Button
                    ...
                    disabled={isToggling}
                  >
                    <Star className={`h-4 w-4 ${item.is_showcase ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground'}`} />
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
      ...
      {mediaItems.length === 0 && !isLoading && (
        <p className="text-center text-muted-foreground">Kütüphanenizde henüz medya bulunmuyor.</p>
      )}
